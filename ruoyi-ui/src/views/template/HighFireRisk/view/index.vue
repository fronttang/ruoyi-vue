      ...
      <el-table-column label="操作" fixed="right" align="center" width="220" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleFirstView(scope.row)"
          >一级展示模块</el-button>
          <el-button
            size="mini"
            type="text"
...
export default {
  ...
  dicts: ['intuitive_detect_type', 'high_risk_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    handleFirstView(row){
      ...
      const params = {};
      this.$tab.openPage("一级展示模块", '/template/HighFireRisk/view/firstlevel/' + templateId + '/' + unitType + '/' + detectId, params);
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除检测表编号为"' + ids + '"的数据项？').then(function() {
