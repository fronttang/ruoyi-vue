...
public class IntuitiveDetectDataServiceImpl implements IIntuitiveDetectDataService {
	...
	public int insertIntuitiveDetectData(IntuitiveDetectData intuitiveDetectData) {
		...
		if ("2".equalsIgnoreCase(intuitiveDetectData.getType())
				|| (intuitiveDetectData.getDetectTitle() != null && 0 == intuitiveDetectData.getDetectTitle())) {
	...
	@Override
	public int updateIntuitiveDetectDataView(IntuitiveDetectData intuitiveDetectData) {
		intuitiveDetectData.setUpdateTime(DateUtils.getNowDate());
		return intuitiveDetectDataMapper.updateIntuitiveDetectData(intuitiveDetectData);
	}
	...
	public int updateIntuitiveDetectData(IntuitiveDetectData intuitiveDetectData) {
		...
		List<IntuitiveDetectDanger> dbDangers = intuitiveDetectDangerMapper
				.selectIntuitiveDetectDangersByDataId(intuitiveDetectData.getId());
		Map<Long, IntuitiveDetectDanger> dangerMap = new HashMap<Long, IntuitiveDetectDanger>();
		if (!CollectionUtils.isEmpty(dbDangers)) {
			dangerMap.putAll(
					dbDangers.stream().collect(Collectors.toMap(IntuitiveDetectDanger::getId, Function.identity())));
		}
		if ("2".equalsIgnoreCase(intuitiveDetectData.getType())
				|| (intuitiveDetectData.getDetectTitle() != null && 0 == intuitiveDetectData.getDetectTitle())) {
			List<IntuitiveDetectDanger> dangers = intuitiveDetectData.getDangers();
	...
	public List<IntuitiveDetectData> selectReportIntuitiveDetectDataList(Long templateId, Long unitId) {
		return intuitiveDetectDataMapper.selectReportIntuitiveDetectDataList(templateId, unitId);
	}
}
